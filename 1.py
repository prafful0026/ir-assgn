...
class Fabrik2D(FabrikBase):
    def _init_(self, joint_positions: List[Vector2], tolerance: float = 0.0) -> None:
        self.joints: List[Vector2] = joint_positions
        print(self.joints)
        super()._init_(joint_positions, tolerance)
...
class Fabrik3D(FabrikBase):
    def _init_(self, joint_positions: List[Vector3], tolerance: float = 0.0) -> None:
...
_all_ = ["Fabrik", "Fabrik2D", "Fabrik3D"]